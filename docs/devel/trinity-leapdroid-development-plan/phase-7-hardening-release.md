...
- Run the matrix under Core and Compatible/microG profiles, and on Windows x64 under Native Bridge `Off`, Houdini, and `libndk_translation` wherever legally enabled.
- Tag every result with package version, APK ABI set, microG APIs used, provider/version/hash, translated ABI, and whether failure reproduces with a native-ABI APK.
- Maintain a separate 50-app blocking/250-app TV matrix tagged with Leanback support, focus behavior, input device, display profile, codec/backend, subtitles, audio layout, microG state, and DRM requirement.
...
- F-Droid is available; microG and user-supplied ARM-on-x64 providers are preview features with published limits. No promise of GMS, Play Integrity, DRM, camera, or snapshot reliability.
...
- F-Droid update flows and the microG Compatible profile are supported; Native Bridge providers graduate individually only after legal, security, and compatibility thresholds pass.
...
- Core and Compatible profiles have independent compatibility results; each production Native Bridge provider has upgrade/disable/rollback procedures and an explicit end-of-support rule.
