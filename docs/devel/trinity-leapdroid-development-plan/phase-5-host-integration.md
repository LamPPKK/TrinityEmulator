...
- Display immutable GApps provider class and status: `UserImport — Uncertified` or the exact partner-approved certification label. Show Play Integrity/legacy SafetyNet/Widevine/banking test status only from signed release metadata; never infer certification from installed packages, a passing client-side check, or user-editable properties.
