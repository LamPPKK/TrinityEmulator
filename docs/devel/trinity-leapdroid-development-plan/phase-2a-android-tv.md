...
- Share kernel, vendor virtual hardware, Gfxstream, ANGLE, protocol, guest agents, microG/F-Droid packages, security policy, and update infrastructure with Desktop editions wherever compatibility permits.
...
## F-Droid, microG, and app policy

- Ship the same Core and Compatible profiles as Desktop: F-Droid in both, microG opt-in in Compatible, and no GApps/Play Store/Google TV packages.
- Test the official F-Droid Client for D-pad navigation, focus visibility, 10-foot text, repository management, install confirmation, updates, and error recovery.
...
- Maintain separate TV compatibility results for F-Droid, sideloaded APKs, microG APIs, Native Bridge provider/ABI, graphics backend, codec path, resolution, and input device.
...
- Publish per-host codec, resolution, refresh, HDR, audio-channel, controller, microG, ARM translation, and DRM support matrices.
...
- Packet-capture long-play idle/navigation/media sessions and verify zero telemetry plus only declared application/F-Droid/opt-in microG/update traffic. Measure sandbox helper wakeups, CPU, memory, dropped frames, and A/V drift.
...
- Provisioning, launcher, Settings, F-Droid, permission prompts, update/recovery, and core apps are fully operable with D-pad only.
...
- No release material implies Google TV certification, Play Store access, Widevine L1, HDCP, or commercial streaming compatibility.
