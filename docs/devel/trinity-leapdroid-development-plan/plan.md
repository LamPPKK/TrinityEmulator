...
- Microsoft has not published the WSA runtime, host shell, integration services, graphics stack, or Widevine implementation. Use WSA as a documented behavior/UX reference only. Its public roadmap confirms ClearKey/MPEG-DASH and software Widevine L3 while marking hardware DRM unavailable; reproduce that capability boundary independently through licensed AOSP/Widevine interfaces. Audit the separately published WSA Linux kernel and reuse only relevant, licensed kernel changes after rebasing them onto the selected current AOSP/GKI kernel.
...
- The archived WSA roadmap explicitly lists ClearKey/MPEG-DASH and software DRM with Widevine L3 as available and hardware DRM as unavailable. The public repo contains no MediaDrm/Crypto HAL, CDM, provisioning, key storage, secure decoder, or protected-output implementation, so WSA supplies a behavioral L3 baseline—not reusable Widevine code or assets.
...
- Add a host-backed MediaCodec path: Media Foundation/D3D11 video on Windows and VideoToolbox/Metal on macOS, with software decode fallback. Use WSA's documented software Widevine L3 capability as the first DRM behavior target, implemented independently through AOSP MediaDrm/Crypto and a licensed CDM/provisioning path. L3 may use only policy-approved non-secure decode/surfaces; secure-buffer requests fail closed. HDCP, Widevine L1, and hardware DRM remain separate certified-partner work.
...
| DRM/protected playback | ClearKey/MPEG-DASH proof | Licensed Widevine L3 proof modeled on WSA behavior | L3 only on approved provider/target/service matrix | Widevine L1/HDCP/hardware DRM certification |
...
| WSA Widevine L3 is mistaken for reusable source or hardware DRM | Unauthorized binary/key reuse or false L1 expectations | Treat the WSA roadmap as behavior evidence only; no runtime/CDM source is public; build licensed AOSP MediaDrm/Crypto L3 independently; keep L1/HDCP out until a separate OEMCrypto/TEE certification passes |
