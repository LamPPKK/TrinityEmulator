...
- Treat the WSA Widevine L3 path as a behavior reference, not a dependency. Verify that no WSA MSIX/guest/CDM/key/provisioning artifact enters source, build, update, or test systems; the shipping L3 provider must come from the approved Widevine partner channel and use independently implemented AOSP MediaDrm/Crypto integration.
...
- For software Widevine L3, preserve the provider's approved CDM signing/update/provisioning and robustness terms, isolate license/key storage per instance, and prevent decrypted media, licenses, keys, device blobs, or provisioning material from entering host logs, captures, clipboard, diagnostics, snapshots, or crash dumps.
