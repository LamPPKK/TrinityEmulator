...
- [ ] Use WSA's documented ClearKey/MPEG-DASH and software Widevine L3 support as a behavior target only; independently implement licensed AOSP MediaDrm/Crypto + CDM/provisioning, and never extract WSA runtime, guest, CDM, device blobs, or keys.
- [ ] Gate Widevine L3 per target with license lifecycle, CENC, non-secure decode/surface, update/reset, multi-instance isolation, output-policy and secret-free diagnostics tests; keep Widevine L1/HDCP/hardware DRM behind a separate OEMCrypto/TEE certification path.
